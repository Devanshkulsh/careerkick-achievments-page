...
const CardSwap: React.FC<CardSwapProps> = ({
  ...
  const isInViewRef = useRef(!animateInViewOnly);
  ...
  useEffect(() => {
    ...
    const scheduleNextSwap = (wait = delay) => {
      clearNextSwap();
      timeoutRef.current = window.setTimeout(playNextSwap, wait);
    };

    const buildSwapTimeline = () => {
    ...
    const playNextSwap = () => {
      if (stopped || isRunningRef.current) return;
      if (animateInViewOnly && !isInViewRef.current) return;

      const tl = buildSwapTimeline();
      if (!tl) return;
