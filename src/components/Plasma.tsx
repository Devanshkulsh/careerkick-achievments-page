...
export const Plasma: React.FC<PlasmaProps> = ({
  ...
  useEffect(() => {
    ...
    const loop = (t: number) => {
      ...
      const dt = Math.min((t - lastTime) * 0.001, 1 / 20);
      lastTime = t;

      if (mouseInteractive) {
        const follow = 1 - Math.exp(-dt * 14);
        mousePos.current.x += (mouseTarget.current.x - mousePos.current.x) * follow;
        mousePos.current.y += (mouseTarget.current.y - mousePos.current.y) * follow;
