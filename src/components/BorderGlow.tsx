...
const BorderGlow: React.FC<BorderGlowProps> = ({
  ...
  const isVisible = isHovered || sweepActive || active;
  const visibleEdgeProximity = isVisible
    ? Math.max(edgeProximity, 1)
    : edgeProximity;
  const borderOpacity = isVisible
    ? Math.max(
        0,
        (visibleEdgeProximity * 100 - colorSensitivity) /
  ...
  const glowOpacity = isVisible
    ? Math.max(
        0,
        (visibleEdgeProximity * 100 - edgeSensitivity) /
          (100 - edgeSensitivity),
      )
    : 0;
